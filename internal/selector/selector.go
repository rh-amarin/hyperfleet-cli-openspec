...
// header is shown above the item list; pass "" to omit it.
// Returns index -1 and nil error when the user aborts (Esc / Ctrl+C).
...
func (FuzzyPreviewSelector) SelectWithPreview(items []Item, preview func(i int) string, header string) (int, error) {
	...
	if header != "" {
		opts = append(opts, fuzzyfinder.WithHeader(header))
	}
	idx, err := fuzzyfinder.Find(items, func(i int) string { return items[i].Name }, opts...)
	if err == fuzzyfinder.ErrAbort {
		return -1, nil
	}
	return idx, err
