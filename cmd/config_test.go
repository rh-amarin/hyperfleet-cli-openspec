...
func TestConfigShow_StateVariables(t *testing.T) {
	...
	// Write state with cluster-id and cluster-name.
...
func TestConfigGet_NoArgs_ShowsHelp(t *testing.T) {
	...
	if !strings.Contains(out, "Usage:") && !strings.Contains(out, "get") {
		t.Errorf("expected help output when no args given, got: %q", out)
	}
}
